...
void parse_arguments(const std::vector<std::string> &input_args, arguments &args)
{
  ...
  {
    Rcpp::Rcout << "Debug: No min-read-counts provided. Defaulting to 0 for all files." << std::endl;
  ...
  {
    Rcpp::Rcout << "Debug: Fewer min-read-counts provided than sequence files. Filling with defaults." << std::endl;
...
int program_main(arguments arguments)
{
  ...
  //(arguments.outFunc)(arguments, output.str(), output_file);
  // go through it
  Rcpp::Rcout << "Debug: go thru it????" << std::endl;
  int ret;
  int tid, pos, vcf_tid;
  ...
  Rcpp::Rcout << "Debug: ready to loop." << std::endl;
  while (bcf_sr_next_line(vcfReader))
  {
    ...
    while ((ret = bam_mplp_auto(iter, &tid, &pos, n_plp, plp)) > 0)
    {
      ...
      {
        Rcpp::Rcout << "Debug: found a thing!!." << std::endl;
        Rcpp::Rcout << "Debug: Found matching position: tid=" << tid
                    << ", pos=" << pos << ", vcf_tid=" << vcf_tid
                    << ", vcf_pos=" << line->pos << std::endl;
        ...
        for (i = 0; i < n; ++i)
        {
          Rcpp::Rcout << "Debug: Processing file " << i + 1 << " of " << n << std::endl;
          ...
          Rcpp::Rcout << "Debug: Set file info" << std::endl;

          Rcpp::Rcout << "Debug: Enter IF: nplp[i] " << n_plp[i] << std::endl;

          Rcpp::Rcout << "Debug: Checking min_read_counts vector size: " << arguments.min_read_counts.size() << std::endl;
          for (size_t idx = 0; idx < arguments.min_read_counts.size(); ++idx)
          {
            Rcpp::Rcout << "Debug: min_read_counts[" << idx << "] = " << arguments.min_read_counts[idx] << std::endl;
          }

          Rcpp::Rcout << "Debug: Enter IF: min read counts " << arguments.min_read_counts[i] << std::endl;

          if (n_plp[i] >= arguments.min_read_counts[i])
          {
            Rcpp::Rcout << "Debug: Sufficient reads for file " << i + 1 << std::endl;
          ...
          {
            Rcpp::Rcout << "Debug: Insufficient reads for file " << i + 1 << std::endl;
            fails_min = true;
            Rcpp::Rcout << "Debug: Insufficient reads for file " << i + 1 << std::endl;
          }
          ...
          Rcpp::Rcout << "Debug: File " << i + 1 << " processed. Refs="
                      << this_file.refs << ", Alts=" << this_file.alts
                      << ", Errors=" << this_file.errors
                      << ", Deletions=" << this_file.deletions << std::endl;
        }
        Rcpp::Rcout << "Debug: end for loop" << std::endl;
  ...
  Rcpp::Rcout << "Debug: in our of read loop." << std::endl;
